   ...
   [Serializable]
    ...
    public class QcDisk
    {
        ...
        private static HardDiskInfo GetHddInfo9x(byte driveIndex)
        {
            ...
            {
                throw new Exception("打开 smartvsd.vxd 失败.");
            }
            ...
            {
                CloseHandle(hDevice);
                throw new Exception("设备控制器失败:DFP_GET_VERSION");
            }
            // If IDE identify command not supported, fails
            if (0 == (vers.fCapabilities & 1))
            {
                CloseHandle(hDevice);
                throw new Exception("错误: IDE 查询命令不支持");
            }
            if (0 != (driveIndex & 1))
            {
            ...
            if (0 != (vers.fCapabilities & (16 >> driveIndex)))
            {
                ...
                CloseHandle(hDevice);
                throw new Exception(string.Format("设备 {0} 是 ATAPI 设备, 不支持", driveIndex + 1));
            }
            else
            {
            ...
            {
                CloseHandle(hDevice);
                throw new Exception("设备控制器失败: DFP_RECEIVE_DRIVE_DATA");
            }
            CloseHandle(hDevice);
        ...
        private static HardDiskInfo GetHddInfoNT(byte driveIndex)
        {
            ...
            {
                CloseHandle(hDevice);
                throw new Exception(string.Format("磁盘 {0} 可能不存在.", driveIndex + 1));
            }
            // If IDE identify command not supported, fails
            if (0 == (vers.fCapabilities & 1))
            {
            ...
            if (0 != (vers.fCapabilities & (16 >> driveIndex)))
            {
                // We don't detect a ATAPI device.
                CloseHandle(hDevice);
                throw new Exception(string.Format("磁盘 {0} 是 ATAPI 设备, 不支持.", driveIndex + 1));
            }
            else
            {
                inParam.irDriveRegs.bCommandReg = 0xec;
