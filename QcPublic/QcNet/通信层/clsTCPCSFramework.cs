...
using System.Collections.Concurrent;
    namespace QcNet
    {
        ...
        public class TcpServer
        {
            ...
            /// 默认的服务器最大连接客户端端数据
            /// </summary>
            public const int DefaultMaxClient = 100000;
            ...
            /// 保存所有客户端会话的哈希表
            /// </summary>
            private  ConcurrentDictionary<SessionId,Session>  _sessionTable;            
            ...
            public TcpServer(ushort port, int maxClient, Coder coder)
            {
                _port = port;
            ...
            /// <param name="port">服务器端监听的端口号</param>
            /// <param name="maxClient">服务器能容纳客户端的最大能力</param>
            public TcpServer(ushort port, int maxClient)
            {
                _port = port;
                _maxClient = maxClient;
            ...
            /// </summary>
            public ConcurrentDictionary<SessionId,Session> SessionTable
            {
                get
                {
            ...
            public virtual void Start()
            {
                ...
                _sessionTable = new ConcurrentDictionary<SessionId, Session>();

                _recvDataBuffer = new byte[DefaultBufferSize];
                IPAddress v = null;
                if (Socket.OSSupportsIPv6)
                {
                    _svrSock = new Socket(AddressFamily.InterNetworkV6,
                ...
                _svrSock.SetKeepAlive(20000, 5000);
                //绑定端口
                IPEndPoint iep = new IPEndPoint(v, _port);
                _svrSock.Bind(iep);
            ...
            public virtual void CloseSession(Session closeClient)
            {
                try
                {
                    ...
                    if (closeClient != null)
                    {
                        ...
                        Session s = null;
                        _sessionTable.TryRemove(closeClient.ID,out s);
            ...
            protected virtual void AcceptConn(IAsyncResult iar)
            {
                ...
                else
                {
                    ...
                    _sessionTable.TryAdd(newSession.ID, newSession);
            ...
            private Session FindSession(Socket client)
            {
                ...
                if (_sessionTable == null) return null;
            ...
            protected virtual void SendDataEnd(IAsyncResult iar)
            {
                ...
                {
                    if (client != null)
                        this.CloseClient(client, Session.ExitType.ExceptionExit);
                }
            }
        ...
        public class TcpClient 
        {
            ...
            public virtual bool Connect(string ip, int port)
            {
                ...
                Socket newsock = new Socket(iep.AddressFamily,
                 SocketType.Stream, ProtocolType.Tcp  );                
                IAsyncResult iar=newsock.BeginConnect(iep, new AsyncCallback(Connected), newsock);
